export const HEADER_PARAMS_RX = /(^|[;,]?\s{0,})[\w\-_]+=/;
...
export function getHeader(headers: IHeader[], name: string, parseParams: boolean = false) {
  name = name.toLowerCase();
  const header = headers.find((h) => h.name.toLowerCase() === name);
  if (header && header.params === null && parseParams) {
    const { params, value } = parseHeaderValueParams(header.value);
    return {
      name,
      params,
      value,
...
export function serializeHeaderParams(params: Record<string, string | undefined | null>) {
  const parts: string[] = [];
